...
std::vector<torch::Tensor> local_std_forward_cuda(torch::Tensor image, torch::Tensor ones_kernel, double eps) {
    ...
    // 2. FFT
    auto sums = fft_cc_forward_cuda(input_stack, kernel_stack);
...
torch::Tensor local_std_backward_cuda(torch::Tensor grad_output, torch::Tensor image, torch::Tensor ones_kernel, torch::Tensor std_map, torch::Tensor s1, double eps) {
    ...
    // 1. Prepare Grads (dS1, dS2)
    auto grad_s1 = torch::empty_like(s1);
    auto grad_s2 = torch::empty_like(s1);
    int numel = s1.numel();
    int threads_p = 1024;
    ...
    auto grad_stack = torch::cat({grad_s1, grad_s2}, 0);
    auto input_stack_dummy = torch::cat({image, image}, 0);
    auto kernel_stack = torch::cat({ones_kernel, ones_kernel}, 0);

    auto d_inputs_vec = fft_cc_backward_cuda(grad_stack, input_stack_dummy, kernel_stack);
