...
std::vector<torch::Tensor> local_std_forward_cpu(torch::Tensor image, torch::Tensor ones_kernel, double eps) {
    ...
    // 1. Prepare Inputs
    // Batched FFT requires contiguous memory, CAT is unavoidable unless FFT supports strided (usually does not for batch)
    auto image_sq = image.square();
    auto input_stack = torch::cat({image, image_sq}, 0);
    // Reuse kernel memory if possible? fft_cc might need expansion. 
    // Optimization: If fft_cc supports broadcasting, we avoid cat kernel. But sticking to existing constraints.
    auto kernel_stack = torch::cat({ones_kernel, ones_kernel}, 0);

    // 2. Compute Sums via FFT
    auto sums = fft_cc_forward_cpu(input_stack, kernel_stack);
    ...
    // Return s1 (contiguous copy if we made one? No, return original view to keep graph clean or copy? 
    // Autograd saves s1. If we return 's1' linked to 'sums', 'sums' stays alive. 
    // Optimally, return 's1_c' or 's1'. 'sums' is large (S1+S2). We only need S1.
    // If we return 's1' (view), 'sums' stays alive. S2 stays alive implicitly. 
    // Memory Optimization: Clone S1 to free S2 memory! S2 is garbage now.
    // Ideally: return s1.clone().
    return {out, s1.clone()};
}
...
torch::Tensor local_std_backward_cpu(torch::Tensor grad_output, torch::Tensor image, torch::Tensor ones_kernel, torch::Tensor std_map, torch::Tensor s1, double eps) {
    ...
    // Optimization: Allocate 'grad_stack' once (2B) and view into it.
    // Reduces CAT overhead later.
    // grad_stack size: 2*B x C x H x W (same as s1 * 2)
    
    auto s1_c = s1.contiguous();
    auto std_map_c = std_map.contiguous();
    auto grad_out_c = grad_output.contiguous();

    // shape of s1 is (B, ...), stack needs (2B, ...)
    ...
    auto grad_s1 = chunks[0]; // View into stack
    auto grad_s2 = chunks[1]; // View into stack
    ...
    // fft_cc_backward handles the heavy lifting
    auto input_stack_dummy = torch::cat({image, image}, 0); 
    auto kernel_stack = torch::cat({ones_kernel, ones_kernel}, 0);
    // grad_stack is already ready!

    auto d_inputs_vec = fft_cc_backward_cpu(grad_stack, input_stack_dummy, kernel_stack);
    auto d_inputs = d_inputs_vec[0]; // (2B, ...)

    // 4. Combine (dI = dI_s1 + 2*I*dI_s2)
    // Fused loop again for memory efficiency (avoid temp tensor for '2*I*dI_s2')
