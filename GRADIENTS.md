...
$$ \frac{\partial L}{\partial I} = \frac{\partial L}{\partial O} * T $$
(Where $*$ denotes full convolution/correlation depending on convention).
...
The ZNCC score at $u$ is the dot product of the standardized vectors divided by $N$ (or sometimes just the dot product, but given the definition usually includes $1/\sigma$, we follow the definition in the codebase):
...
1.  Backpropagate $\delta(u)/\sigma_{I_u}$ through the correlation with $\hat{T}$.
2.  Subtract the backpropagation of $(\delta(u) \cdot Z(u) / (N \sigma_{I_u}))$ through the "local standardization" operation (conceptually similar to instance norm backward).

#### Gradient w.r.t Template ($T$)

$Z(u)$ depends on $T$ via $\hat{T}(v)$. $\hat{T}$ depends on $T$ via global standardization.
$$ Z(u) = \sum_v \hat{I}_u(v) \hat{T}(v) \cdot \frac{1}{N} $$
Wait, strict definition check:
The formula used was $Z(u) = \frac{1}{\sigma_{I_u}} \sum I(u+v) \hat{T}(v)$.
Which is equivalent to $Z(u) = \sum \hat{I}_u(v) \hat{T}(v)$?
$\sum \hat{I}_u \hat{T} = \sum \frac{I-\mu_I}{\sigma_I} \hat{T} = \frac{1}{\sigma_I} (\sum I \hat{T} - \mu_I \sum \hat{T}) = \frac{1}{\sigma_I} \sum I \hat{T}$.
Yes. So $Z(u)$ is the dot product of standardized patches.
...
$$ \hat{T}(v) = \frac{T(v) - \mu_T}{\sigma_T} $$
The Jacobian of Standardization is:
$$ \frac{\partial \hat{T}(v)}{\partial T(j)} = \frac{1}{\sigma_T} \left( \mathbb{1}_{v=j} - \frac{1}{N} - \frac{\hat{T}(v) \hat{T}(j)}{N} \right) $$
(Standard derivation for Batch/Layer Norm).

So:
$$ \frac{\partial L}{\partial T(j)} = \sum_v \frac{\partial L}{\partial \hat{T}(v)} \frac{\partial \hat{T}(v)}{\partial T(j)} $$
$$ = \frac{1}{\sigma_T} \left( \frac{\partial L}{\partial \hat{T}(j)} - \frac{1}{N} \sum_v \frac{\partial L}{\partial \hat{T}(v)} - \frac{\hat{T}(j)}{N} \sum_v \frac{\partial L}{\partial \hat{T}(v)} \hat{T}(v) \right) $$

Let $G_{\hat{T}} = \nabla_{\hat{T}} L$.
$$ \nabla_T L = \frac{1}{\sigma_T} \left( G_{\hat{T}} - \text{mean}(G_{\hat{T}}) - \hat{T} \cdot \text{mean}(G_{\hat{T}} \odot \hat{T}) \cdot N \right) $$
Actually $\sum G \hat{T} / N = \text{mean}(G \odot \hat{T})$.
